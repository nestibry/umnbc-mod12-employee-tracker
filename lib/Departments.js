...
async function viewUtilizedBudgetByDepartment() {
    return new Promise((resolve, reject) => {
        db.execute(`SELECT d.name, SUM(r.salary) AS utilized_budget FROM roles r INNER JOIN departments d ON r.department_id = d.id GROUP BY r.department_id;`, (err, data) => {
            if (err) reject({status: "error", body: err});
            resolve({status: "success", body: data});
        });
    });
